...
Polygon = list[
    int
]  # List/array like [x1, y1, x2, y2], above polygon would be [1, 1, 1, 2, 300, 300, 3, 3]
...
def encode_polygon(polygon: Polygon, tile_size: int):
    ...
    num_points_in_tile = []
    x_jumps, y_jumps = [], []
    remainders = array.array(
        "B"
    )  # Init byte array for remainders, so tile_size <= 256!
...
def polygons_2_bytes(polygons: EfficientArray, tile_size=256):
    ...
    dump_2_disk(
        write_handle, polygon_lengths, tile_size, num_rows, num_cols, encoded_polygon
    )
